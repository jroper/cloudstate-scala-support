...
import io.cloudstate.scalasupport.Jsonable
...
class AnySupport(descriptors: Array[Descriptors.FileDescriptor],
      ...
      .getOrElseUpdate(
        ...
        Try {
          try {
            ...
            if (classOf[Jsonable].isAssignableFrom(jsonClass)) {
              throw SerializationException(
  ...
  def encodeScala(value: Any): ScalaPbAny =
    value match {
      ...
      case other =>
        throw SerializationException(
          s"Don't know how to serialize object of type ${other.getClass}. Try passing a protobuf, using a primitive type, or using a subclass of Jsonable."
        )
    }
